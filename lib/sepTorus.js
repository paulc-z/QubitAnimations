...
const div_width  = 600;
const div_height = div_width;
const width_padding = 100;
const height_padding  = 100;
const width  = (div_width - width_padding )/4; 
const height = (div_width - height_padding)/4;
const slider_width = div_width;
const statepoint_radius = 4;
...
const stateline_color      = "rgb(0, 255, 0)"
const bloch1_color         = "rgb(34,139,34)" // or dark brown for #876543
const bloch2_color         = "rgb(0,0,0)"
const l_crossSection_color = "#5599bb"
...
var container = document.getElementById('div_sepTorus');
...
var m_material = new THREE.MeshPhongMaterial({ color: 0xf7e297, transparent: true, opacity: 0.3, reflectivity: 1 });
var m_torus = new THREE.Mesh(m_geometry, m_material);
scene.add(m_torus);


// plot radial axes that intersects at the up up state
var geometry = new THREE.CylinderGeometry(r1+1 , r1+1 , 5, 100, 10, true);
...
p.setAttribute("id", "sepTorus_state")
document.getElementById("div_sepTorus").append(p);
...
function torus_2q(state2q){
    ...
    */
    ...
    // cross section
    var geometry = new THREE.CylinderGeometry(r1 , r1 , 0.1, 100);
    var material = new THREE.MeshBasicMaterial({ color: l_crossSection_color, transparent: false, side: THREE.DoubleSide });
    ...
    if (Math.abs(radius) <= 0.001) {
        ...
        document.getElementById("torus_state").innerHTML = "Entangled r=0 s= " + s.toString();
    }
    else {
        // state text
    ...
    camera.lookAt(m_torus.position);
    camera.updateMatrix();
...
function torus_addpoint(state2q){
    /* 
    Plots a two qubit state with 3 params specified. 
    t1 is the angle of the 1st qubit on the bloch circle.
    t2 is the angle of the 2nd qubit on the bloch circle.
    ...
    */
    ...
    if (Math.abs(radius) <= 0.001) {
        ...
        // state arrow pointer 
        let sign = 1;
        if (sign > 0) s / Math.abs(s);
        var origin = new THREE.Vector3((r + (1 + .5 * sign) * r1 * Math.cos(theta2)) * Math.cos(theta1), (r + (1 + .5 * sign) * r1 * Math.cos(theta2)) * Math.sin(theta1), (1 + .5 * sign) * r1 * Math.sin(theta2));
        var dir = new THREE.Vector3(-Math.cos(theta2) * Math.cos(theta1),
            -Math.cos(theta2) * Math.sin(theta1),
            -Math.sin(theta2));
        dir.multiplyScalar(sign);
...
torus_addpoint(ddState);
torus_addpoint(udState);
torus_addpoint(duState);
torus_addpoint(uuState);
...
// Torus cross Section 
var svg_torusCrossSection = d3.select("#div_sepTorusXsec").append("svg")
    .attr("width", width)
    .attr("height", height);
...
var svg_torusTopView = d3.select("#div_sepTorusTopView").append("svg")
    .attr("width", width)
    .attr("height", height);
...
function torus_topView(state2q) {
    ...
    var arc = d3.arc()
        ...
        .outerRadius(bloch_r/2)
        .startAngle( 0) //converting from degs to radians
        .endAngle(Math.PI*2); //just radians

    svg_torusTopView.append("path")
        .attr("fill", "white")
        .attr("d", arc)
    ...
    arc = d3.arc()
        .innerRadius(bloch_r*2/4)
        .outerRadius(bloch_r*3/4)
        .startAngle( 0) //converting from degs to radians
    ...
    arc = d3.arc()
        .innerRadius(bloch_r*3/4)
        .outerRadius(bloch_r*5/4)
        .startAngle(0) //converting from degs to radians
        .endAngle(2*Math.PI); //just radians
    ...
    arc = d3.arc()
        .innerRadius(bloch_r*5/4)
        .outerRadius(bloch_r*6/4)
        .startAngle(0) //converting from degs to radians
        .endAngle(2*Math.PI); //just radians
    ...
    arc = d3.arc()
        .innerRadius(bloch_r*6/4)
        .outerRadius(bloch_r*7/4)
        .startAngle( 0) //converting from degs to radians
        .endAngle(Math.PI*2); //just radians
...
var svg_blochCircle1 = d3.select("#div_sepQubit1").append("svg")
    .attr("id", "blochCircle1")
    .attr("width", width)
    .attr("height", height);

var svg_blochCircle2 = d3.select("#div_sepQubit2").append("svg")
...
// var checkBoxes = ["upup", "updown", "downup", "updown"]
// for (let i = 0 ; i < checkBoxes.length ; i++){
//     var checkBox = "#" + checkBoxes[i];
//     d3.select(checkBox).on("change",update(checkBox));
//     update();
// }
