...
var svgContainer = d3.select("#mixedstateBC").append("svg")
      .attr("id", "svgIn")
...
zeropercentstatement
      ...
      .text(" state is ")
zeropercentstatement.append('tspan')
  .attr('id','zeroprobabilityMixed').style('fill',zerocolor)
...
onepercentstatement.append('tspan').attr('id', 'oneprobabilityMixed').style('fill', onecolor)
...
function displayBC(mx,my,axismoving){
      ...
      d3.select("#zeroprobabilityMixed").text(String(math.round(100*(100-oneprob))/100))
      d3.select("#oneprobabilityMixed").text(String(math.round(100*oneprob)/100))
}
